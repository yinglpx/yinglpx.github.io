...
class SAM3App {
    ...
    async generate3D() {
        ...
        try {
            ...
            if (data.success) {
                ...
                if (data.model_data) {
                    await this.loadModelFromDataURL(data.model_data, format);
                }
    ...
    // 从Data URL加载模型的方法
    async loadModelFromDataURL(dataURL, format) {
        if (!this.threeInitialized) {
    ...
    async download3D() {
        ...
        try {
            ...
            // 发送请求获取模型文件
            const response = await fetch(`${this.apiBase}/triposg/download`, {
                method: 'GET'
            });
